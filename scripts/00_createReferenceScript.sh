...
state_script_path="../contracts/state_contract.plutus"
mint_script_path="../contracts/mint_contract.plutus"

# Addresses
reference_address=$(cat wallets/reference-wallet/payment.addr)

state_min_utxo=$(${cli} transaction calculate-min-required-utxo \
    --babbage-era \
    --protocol-params-file tmp/protocol.json \
    --tx-out-reference-script-file ${state_script_path} \
    --tx-out="${reference_address} 5000000" | tr -dc '0-9')
echo "Lock Min Fee" ${state_min_utxo}
...
echo
state_value=$state_min_utxo
mint_value=$mint_min_utxo
state_script_reference_utxo="${reference_address} + ${state_value}"
mint_script_reference_utxo="${reference_address} + ${mint_value}"

echo -e "\nCreating Script Reference:\n" ${state_script_reference_utxo}
...
${cli} transaction build-raw \
    ...
    --tx-out="${state_script_reference_utxo}" \
    --tx-out-reference-script-file ${state_script_path} \
...
firstReturn=$((${starting_reference_lovelace} - ${state_value} - ${mint_value} - ${fee}))
